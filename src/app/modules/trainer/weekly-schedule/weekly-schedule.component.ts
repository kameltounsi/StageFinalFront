...
import { WeeklyScheduleService, WeeklyItem } from './weekly-schedule.service';
...
export class WeeklyScheduleComponent implements OnInit {
    ...
    // Table data
    days = signal<Date[]>([]); // Monday -> Sunday
    grid = signal<WeeklyItem[][]>([]); // flattened [dayIndex * slots + slotIndex] -> items
    ...
    constructor(private svc: WeeklyScheduleService) {}
    ...
    /** Pick a group name regardless of backend shape */
    groupName(it: WeeklyItem): string {
        return it.groupeNom || it.groupe?.nom || it.groupe?.specialite || '—';
    }
    // AJOUT en haut à côté des autres signals
...
// AJOUT dans la classe
    downloadPdf(): void {
        ...
        // Variante 1 (start/end) :
        this.svc.downloadMyWeekPdf(startYmd, endYmd).subscribe({
            next: (blob: Blob) => {
                const a = document.createElement('a');
                a.href = URL.createObjectURL(blob);
                a.download = `weekly_schedule_${startYmd}_to_${endYmd}.pdf`;
                a.click();
