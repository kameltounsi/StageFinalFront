    ...
    signIn(): void {
        ...
        this._authService.signIn(credentials).subscribe(
            (response) => {
                ...
                console.log('Token reçu:', response.token); // ✅ ici, pas response.token !

                Swal.fire({
                    icon: 'success',
                    title: 'Welcome Back!',
                    text: `Hello, ${response.user?.fullName || 'User'}`,
                    confirmButtonColor: '#3085d6',
                });
                // Sauvegarder le token si nécessaire
                localStorage.setItem('token', response.token); // ou dans AuthService
